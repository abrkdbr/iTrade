...
def get_account_info():
    ...
    if not response_content:
        ...
        if response.is_success():
            ...
            print(str(response.code) + "," + response.message + "," + response.data)
...
def trade_apis():
    ...
    order_id_new = order.order_id  # you can operate order via id too
    openapi_client.place_order(order)
    new_order = openapi_client.get_order(order_id=order_id_new) #old:order_id=order.order_id
    print("%$%$%$%$%$%$%$%$%$%$%$%")
    print(new_order)
